...
- **Positioning**: The most feature-complete open-source agent framework. Every feature APEX needs exists somewhere in Hermes (skills, memory, sessions, delegation, MCP, plugins, hooks, gateway, cron, profiles, security, streaming, provider pools). The cost: Python, 2.8M LOC, 10K+ files.
...
- APEX copy: the retry/fallback state machine (`TurnRetryState`); error classification (`FailoverReason`); prompt caching inviolability; the "narrow waist" philosophy (core = small, capability at edges).
- APEX improve: explicit phase state machine (UNDERSTANDING/INVESTIGATING/IMPLEMENTING/VERIFYING/...) with tool policies per phase (APEX §45); the loop should be cleaner in Rust.
...
- APEX copy: aux-model-based summarization with head/tail protection; structured summary (Resolved/Pending); iterative updates; token-budget tail protection.
- APEX improve: adopt jcode's threshold constants (80% soft, 95% hard, keep 10 turns, flat image token cost); add per-model budgets from catalog.
...
- APEX copy: FTS5 session search; dual-store (user profile + personal notes); budget-gated injection; separate memory/skills/prompts directories.
- APEX improve: add ai-memory's compile-not-retrieve pattern; add entity graph; add optional embeddings; add conflict resolution/explicit metadata. APEX's memory should be a module inside the Rust core, not a Python module.
...
- APEX copy: the SKILL.md format (APEX §17); the lifecycle (experience → candidate → reuse → validation → promotion); the skill_manage API.
- APEX improve: versioned skills with verification; dependency resolution; skill testing; skill blocking (mark conflict as BLOCKER). APEX's skill system should be integrated into the Rust core, not Python.
...
- APEX copy: the delegation API (spawn/steer/stop/list, batch, output_schema, leaf/orchestrator, concurrency caps); worktree isolation; typed result schemas.
- APEX improve: make subagent results queryable (APEX §11 — structured schema with findings/files/tests/risks/confidence); separated worktree with conflict detection (APEX §10); explicit subagent budgets.
...
- APEX copy: the hook discipline (only add hooks for concrete consumers); PreToolUse pattern for security gating.
- APEX improve: full event bus (APEX §31: session.started, task.started, tool.started, file.changed, diagnostic.created, test.failed, agent.spawned, memory.created, etc.).
...
- APEX copy: MCP for external extensibility only; core tools are native (APEX §32).
- APEX improve: MCP should be an optional module (not loaded on the hot path).
...
- APEX copy: credential pools + rotation; aux client pattern (separate model for cheap work); model metadata.
- APEX improve: role-based routing (FAST/SMART/CODE/...) with learned per-task-class stats (APEX §3); cheapest-sufficient selection; catalog-driven model metadata.
...
- APEX copy: secret redaction; approval mode + command approval; path restrictions; sanitization pipeline.
- APEX improve: prompt-injection detection on repo content and tool output; permission scopes for subagents; sandbox options.
...
- APEX copy: FTS5 session store; session search; cron scheduling pattern (no_agent mode, monitor scripts, script-based jobs).
- APEX improve: keep session engine in Rust core; make it embeddable (no external process).
...
- APEX copy: doctor command; observability dashboard.
- APEX improve: event-bus-driven telemetry (APEX §31); agent hub + agent event bus.
...
- **Fundamental**: Python is the wrong language for APEX's performance targets. Hermes's architecture DESIGN is the right reference; the IMPLEMENTATION language is wrong for APEX.
...
**Reject**: Python implementation; the 31K-line gateway/run.py; 10K+ file project structure; the bespoke message-alternation invariants (use standard JSON-RPC); the 866K LOC of tests (APEX should be more test-concise).

**Reuse score: 9/10 as design philosophy reference; 1/10 as fork base.** Hermes is conceptually the closest project to APEX's vision, but it's Python and 2.8M LOC. APEX must reimplement the architecture in Rust. The SKILL.md files, delegation API, and session store schema can be directly ported.