...
> Document purpose: Phase 0 research for ByteAi (ByteAi). Verified by reading source, not README alone.
...
- ByteAi copy: daemon-with-thin-clients for multi-session efficiency; headless `run` mode; self-dev reload discipline.
- ByteAi improve: define the client protocol as a typed schema (JSON-RPC-like) instead of bespoke messages.
- ByteAi reject: the sheer size of the client-lifecycle/comm matrix (many bespoke modules) — ByteAi should keep the core loop small and push features to modules.
...
- ByteAi copy: relevance-gated top-k injection; skill-as-memory (synthetic providers); optional lazy embeddings with LRU; cross-encoder reranking; trust/reinforcement metadata.
- ByteAi improve: make the sidecar pluggable (any cheap model); add explicit conflict metadata (supersedes/expiration) like ai-memory.
- ByteAi reject: coupling memory to one model family; unversioned prompt logic.
...
- ByteAi copy: tldr discipline for inter-agent messages; typed completion reports; membership caps + budget gating.
- ByteAi improve: adopt a typed result schema per agent role (status/findings/files/tests/risks) instead of free-form marker strings.
- ByteAi reject: 1,000-member swarms as a target — ByteAi should start with small, genuinely-parallel teams.
...
- ByteAi copy: the threshold ladder, verbatim recent-turn retention, flat image token cost, emergency truncation caps, 413 payload budget.
- ByteAi improve: per-model context budgets from a model catalog; adaptive compaction that measures real provider input tokens.
- ByteAi reject: hardcoded 200k.
...
- ByteAi copy: separate tool schema crate; command-risk scoring for approvals.
- ByteAi improve: a single native tool dispatcher with JSON-schema tool contracts and failure-classification (jcode lacks explicit failure-class taxonomy; ByteAi needs one).
- ByteAi reject: tool impls embedded in server modules (hard to reuse).
...
- ByteAi copy: provider trait boundary; per-provider runtime isolation; doctor/E2E provider tests.
- ByteAi improve: capability roles (FAST/SMART/CODE/DEBUG...) on top of providers; success-rate learning per task class (jcode has no learned routing stats beyond model resolution).
- ByteAi reject: nothing major — but don't copy the provider count; start with OpenAI-compatible + Anthropic + Gemini.
...
- ByteAi copy: fuzzy search; discovery tooling.
- ByteAi improve: add tiered search (literal→regex→symbol→AST→semantic) and a multi-strategy edit engine with LSP validation (both absent here — this is oh-my-pi's strength).
- ByteAi reject: none — absence is a gap to fill, not a pattern to copy.
...
- ByteAi copy: keybinding model, kill ring, session picker, split view, usage overlay (token/context%) — all proven UX.
- ByteAi improve: keep TUI as a client of the core protocol, not a monolith (jcode-tui files are huge: input.rs 4,084, commands.rs 3,664).
- ByteAi reject: 4K-line UI files.

### Telemetry (`jcode-telemetry-core`, `telemetry-worker/`)
- what: separate telemetry worker process; event capture.
- ByteAi copy: out-of-process telemetry; event bus separation (matches ByteAi event bus goal).
- ByteAi reject: none — but ensure telemetry is opt-in (jcode's AGENTS.md-era PRs show scrutiny here).

### Sessions & storage (`jcode-storage`, `jcode-session-types`)
- what: session persistence with durable state; `durable_state.rs` in server.
- ByteAi copy: typed session types; durable state snapshots.
- ByteAi improve: FTS-indexed session search (Hermes/ai-memory pattern).
...
- **Published claims (repo-maintainer, VERIFIED-UNVERIFIED)**:
  ...
  - Time to first frame: jcode 14.0 ms vs pi 590.7, Codex 882.8, OpenCode 1035.9, Claude Code 3436.9 (10 interactive PTY launches) — **UNVERIFIED**; plausible for a native TUI, but must be reproduced independently (ByteAi benchmark suite requirement).
...
- ByteAi copy: maintainer-benchmark skepticism; in-repo benchmark scripts with case files; PTY first-frame measurement technique.
- ByteAi improve: publish methodology + scripts with every claim; measure on macOS (jcode measured Linux).
...
- ByteAi copy: command-risk scoring; 413/payload recovery; reload recovery.
- ByteAi improve: explicit failure-class taxonomy + strategy-switching retries (ByteAi §23).
- ByteAi reject: none major.
...
- No LSP, no DAP, no multi-strategy edit engine, no tiered search (gaps ByteAi must fill from oh-my-pi).
