# ByteAi (ByteAi) — Architecture Comparison & Decision Matrix
...
ByteAi should be a **clean Rust implementation** — not a fork of any reference
...
architecture proven by these projects gives ByteAi the performance mandate
(<100 ms start, <50 MB idle) without inheriting anyone's debt.

## 2. Project-by-project verdict (details in `docs/research/<project>.md`)

| Project | Language | Size (LOC text) | License | Fork score | Role in ByteAi |
|---|---|---|---|---|---|
...
## 4. The ByteAi Architecture (fast core + intelligent orchestration + optional power modules)

```
byteai/  (codename ByteAi)
├── crates/
│   ├── byteai-core/           agent loop, state machine, scheduler, tool dispatcher
│   ├── byteai-provider/       provider traits + OpenAI-compatible/Anthropic/Gemini/Ollama/vLLM runtimes, role router
│   ├── byteai-tools/          native tools: shell, fs, read, search, edit, todo, memory, delegate
│   ├── byteai-edit/           multi-strategy edit engine (exact/contextual/sloppy/AST/whole-file) + LSP validation
│   ├── byteai-lsp/            LSP manager (diagnostics, symbols, refs, rename, formatting, writethrough)
│   ├── byteai-dap/            DAP manager (launch/attach/breakpoints/step/evaluate; debugpy/lldb/gdb/dlv/node)
│   ├── byteai-search/         tiered search: literal → regex → symbol/LSP → AST → optional semantic
│   ├── byteai-read/           progressive-disclosure file reader (symbols, ranges, functions, classes, AST nodes)
│   ├── byteai-session/        session engine + FTS5 store (state.db), session search, resume, handoff
│   ├── byteai-context/        context budget, compaction (jcode ladder), checkpoint/rewind, eviction
│   ├── byteai-memory/         hybrid memory: working state / project wiki / episodic / procedural
│   │                        Markdown source of truth + SQLite indexes (FTS, entities, graph, optional embeddings)
│   ├── byteai-skills/         SKILL.md loader, lesson capture, promotion, verification
│   ├── byteai-subagent/       subagent coordinator: spawn/steer/stop, worktrees, conflict detection, typed results
│   ├── byteai-router/         capability-based model routing with learned per-task-class stats
│   ├── byteai-review/         optional independent reviewer (INFO/WARNING/BLOCKER)
│   ├── byteai-exec/           execution abstraction: local / Docker / SSH / remote sandbox
│   ├── byteai-events/         event bus (session.*, tool.*, file.*, diagnostic.*, test.*, agent.*, memory.*)
│   ├── byteai-security/       redaction, approval policies, path restrictions, prompt-injection detection
│   ├── byteai-telemetry/      metrics, usage, agent hub data
│   ├── byteai-protocol/       typed tool/subagent/result schemas (JSON-RPC-ish, serde)
│   └── byteai-tui/            TUI client (ratatui) over the core protocol
...
### Lineage of each subsystem (what ByteAi copies conceptually from where)

| ByteAi subsystem | Primary source | Also informed by |
|---|---|---|
...
| Capability-based routing | ByteAi spec §2-3 (not in references) | hermes aux client, oh-my-pi catalog |
| Tiered search / progressive reads | ByteAi spec §6-7 (not in references) | oh-my-pi natives |
| Event bus | ByteAi spec §31 | jcode telemetry, hermes hooks |
...
- i-have-adhd: platform-specific hook enforcement (ByteAi enforces output format at the display layer + core invariant).

## 6. Licensing

All six references are MIT. Any code copied verbatim must retain the original
copyright notice (MIT requires it); ByteAi is being written as a clean
...
`NOTICE.md`. ByteAi itself ships MIT.
...
companion process later for heavy cross-harness memory, but ByteAi's core memory
