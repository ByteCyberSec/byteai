...
fn corpus_vectors(entries: &[byteai_memory::Entry]) -> Vec<(i64, HashMap<String, f64>)> {
...
fn score_entries(
    query: &str,
    entries: &[byteai_memory::Entry],
    limit: usize,
) -> Vec<(f64, byteai_memory::Entry)> {
    ...
    let mut scored: Vec<(f64, byteai_memory::Entry)> = entries
        .iter()
        .zip(docs.iter())
...
mod tests {
    ...
    fn entry(id: i64, title: &str, body: &str) -> byteai_memory::Entry {
        byteai_memory::Entry {
            id,
            kind: Kind::Note,
            title: title.into(),
