...
use byteai_lsp::{LspRegistry, LspServer, ServerState, language_for_path};
...
async fn with_server<T>(
    ...
    path: &std::path::Path,
    text: &str,
    f: impl for<'a> FnOnce(&'a mut LspServer) -> byteai_lsp::LspFuture<'a, T>,
) -> Result<T, String> {
...
fn format_diags(diags: &[byteai_lsp::Diagnostic]) -> String {
