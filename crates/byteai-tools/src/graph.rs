...
impl GraphTool {
    ...
    fn collect(root: &Path) -> Vec<Node> {
        ...
        for f in files {
            ...
            let Some(lang) = byteai_ast::language_for_path(&f) else { continue };
            let Ok(syms) = byteai_ast::extract(lang, &text) else { continue };
            for s in syms {
                if matches!(s.kind, byteai_ast::SymbolKind::Import) {
                    continue;
                }
                nodes.push(Node {
                    name: s.name.clone(),
            ...
            match action.as_str() {
                ...
                "files" => {
                    ...
                    for f in &files {
                        ...
                        let Some(_) = byteai_ast::language_for_path(f) else { continue };
                        let Ok(imports) = byteai_ast::imports_file(f) else { continue };
