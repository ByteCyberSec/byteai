...
impl Registry {
    ...
    pub fn builtins(ctx: &ToolContext) -> Self {
        ...
        r.register(Arc::new(terminal::TerminalTool::new(ctx.data_dir.clone())));
        r.register(Arc::new(goal::GoalTool::new(ctx.data_dir.clone())));
        r.register(Arc::new(feedback::FeedbackTool::new(ctx.data_dir.clone())));
        r.register(Arc::new(autoskill::AutoSkillTool::new(ctx.data_dir.clone())));
        r.register(Arc::new(conductor::ConductorTool::new(ctx.data_dir.clone())));
        r.register(Arc::new(autocontext::AutoContextTool::new(ctx.data_dir.clone())));
