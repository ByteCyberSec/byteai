...
//!
//! Doctrine (see docs/byteai-intelligence.md):
...
//!     <data>/intelligence/capabilities.md (mirrored to .byteai/intelligence/).
//!
...
impl GithubTool {
    ...
    fn memory(&self) -> BoxFuture<'_, ToolOutcome> {
        ...
        Box::pin(async move {
            let started = Instant::now();
            let mut out = String::new();
            match byteai_memory::Memory::open(&ctx.data_dir.join("memory")) {
...
fn llm_call(
    ...
    Box::pin(async move {
        ...
        let sys = byteai_types::Message::system(&system);
        let usr = byteai_types::Message::user(&user);
        client
