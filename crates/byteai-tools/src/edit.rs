        ...
        Box::pin(async move {
            ...
            if validate
                && let Some(registry) = &lsp {
                    ...
                    let lang = byteai_lsp::language_for_path(&p);
...
async fn lsp_diag(registry: &LspRegistry, path: &std::path::Path, text: &str, lang: &str) -> anyhow::Result<Vec<byteai_lsp::Diagnostic>> {
    let root = path.parent().map(|p| p.to_path_buf()).unwrap_or_else(|| PathBuf::from("."));
    let state = registry.get(lang, &root).await?;
    let mut st = state.lock().await;
    match &mut *st {
        byteai_lsp::ServerState::Ready(s) => {
        ...
        byteai_lsp::ServerState::Unavailable(reason) => Err(anyhow::anyhow!("unavailable: {reason}")),
        byteai_lsp::ServerState::Spawning => Err(anyhow::anyhow!("still spawning")),
    }
}
