...
use anyhow::Result;
use apex_provider::{pool::ProviderPool, StreamEvent};
...
pub struct AgentConfig {
    ...
    pub tdai: apex_memory::tdai::TdaiConfig,
}
...
impl Default for AgentConfig {
    fn default() -> Self {
        Self {
            ...
            tdai: apex_memory::tdai::TdaiConfig::default(),
        }
    }
}
...
impl Agent {
    ...
    pub fn inject_skills(&mut self, query: &str) {
        ...
        let relevant = apex_tools::skills::find_relevant(&skills_root, query, 5);
    ...
    fn strip_injected_blocks(&mut self) {
        if let Some(apex_types::Message { role: Role::System, content: Some(c), .. }) = self.history.first_mut() {
            const MARKERS: [&str; 3] = [
                "\n\nRelevant prior knowledge:",
                "\n\nRelevant skills for this task:",
    ...
    pub async fn inject_tdai_memory(&mut self, query: &str) {
        ...
        let hub = match apex_memory::hub::MemoryHub::open(&self.data_dir.join("memory")) {
            Ok(h) => h,
            Err(_) => return,
        };
    ...
    pub async fn capture_tdai_turn(&mut self, user_input: &str, assistant_text: &str) {
        ...
        let mut hub = match apex_memory::hub::MemoryHub::open(&self.data_dir.join("memory")) {
            Ok(h) => h,
            Err(_) => return,
        };
        ...
        F2: FnMut(&ToolOutcome) + Send,
    {
        ...
        let mut defs: Vec<apex_types::ToolDef> = if tools_active {
    ...
    async fn extract_memories(&mut self, outcome: &AgentOutcome) {
        ...
        let mut hub = apex_memory::hub::MemoryHub::open(&self.data_dir.join("memory")).ok();
...
mod tests {
    ...
    async fn inject_tdai_memory_pulls_persona_and_scenarios() {
        ...
        {
            let mut hub = apex_memory::hub::MemoryHub::open(&dir.join("memory")).unwrap();
