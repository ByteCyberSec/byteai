...
async fn handle_request(
    ...
    match (method, path) {
        ...
        ("POST", "/v1/chat/completions") => {
            ...
            let byteai_msgs: Vec<byteai_types::Message> = msgs.iter()
                .filter_map(|m| {
                    let role = m.get("role")?.as_str()?;
                    let content = m.get("content")?.as_str()?;
                    Some(match role {
                        "system" => byteai_types::Message::system(content),
                        "user" => byteai_types::Message::user(content),
                        "assistant" => byteai_types::Message::assistant(Some(content.into()), None, None),
                        _ => return None,
                    })
                })
                .collect();
            ...
            let (content, _tcalls, _usage) = match client {
                Some(c) => c.chat(target_model, &byteai_msgs, &[], None).await?,
