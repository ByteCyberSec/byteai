...
impl MemorySection {
    /// Convert to the byteai-memory TDAI config used by the agent core.
    /// Only `enabled` and `conversation_id` matter for the native hub.
    pub fn to_tdai_config(&self) -> byteai_memory::tdai::TdaiConfig {
        byteai_memory::tdai::TdaiConfig {
...
fn default_tool_select_max() -> usize {
    byteai_core::toolselect::DEFAULT_MAX
}
