...
use apex_types::{SessionFile, ToolDef, ToolOutcome};
...
mod tests {
    ...
    fn write_session(dir: &PathBuf, id: &str, messages: &[(&str, &str)]) {
        let msgs: Vec<apex_types::Message> = messages
            .iter()
            .map(|(role, content)| match *role {
                "user" => apex_types::Message::user(content.to_string()),
                "assistant" => apex_types::Message::assistant(Some(content.to_string()), None, None),
                _ => apex_types::Message::system(content.to_string()),
            })
            .collect();
        let sf = SessionFile {
            id: id.into(),
            ...
            provider: "test".into(),
            messages: msgs,
            usage: apex_types::Usage::default(),
        };
        std::fs::write(
