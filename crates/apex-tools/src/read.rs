...
Use symbols first to decide what to read — read less, understand more.".into(),
    ...
    fn execute(&self, args: Value) -> BoxFuture<'_, ToolOutcome> {
        Box::pin(async move {
            ...
            // ── Smart modes via apex-ast ──────────────────────────────────────
            if want_symbols {
                return match apex_ast::symbol_summary(&path, 120) {
            ...
            if want_imports {
                return match apex_ast::imports_file(&path) {
                    Ok(imports) => {
            ...
            if let Some(name) = function {
                let lang = match apex_ast::language_for_path(&path) {
                    Some(l) => l,
                    None => return ok_outcome("", self.name(), format!("ERROR: unsupported language for {}", path.display()), 0),
                };
                return match std::fs::read_to_string(&path) {
                    Ok(text) => match apex_ast::find_definition(lang, &text, &name) {
                        Some(def) => {
